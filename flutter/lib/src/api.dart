...
Future<List<TemplateSummary>> listTemplates() async {
  final res = await http.get(_u('/templates'), headers: _headers());
  if (res.statusCode != 200) throw Exception('HTTP ${res.statusCode}');
  return (jsonDecode(res.body) as List)
...
Future<TemplateDetail> getTemplate(int id) async {
  final res = await http.get(_u('/templates/$id'), headers: _headers());
  if (res.statusCode != 200) throw Exception('HTTP ${res.statusCode}');
  return TemplateDetail.fromJson(jsonDecode(res.body) as Map<String, dynamic>);
}
...
Future<TemplateSummary> createTemplate({
  ...
  final res = await http.post(
    _u('/templates'),
    headers: _headers(),
    body: jsonEncode({'name': name, if (notes != null) 'notes': notes, 'sets': sets}),
  );
  if (res.statusCode != 201) throw Exception('HTTP ${res.statusCode}');
  return TemplateSummary.fromJson(jsonDecode(res.body) as Map<String, dynamic>);
}

Future<void> deleteTemplate(int id) async {
  final res = await http.delete(_u('/templates/$id'), headers: _headers());
  if (res.statusCode != 204) throw Exception('HTTP ${res.statusCode}');
}

Future<void> applyTemplate({required int templateId, required int workoutId}) async {
  final res = await http.post(
    _u('/templates/$templateId/apply/$workoutId'),
    headers: _headers(),
  );
  if (res.statusCode != 204) throw Exception('HTTP ${res.statusCode}');
}
